...
module.exports = router;const router = require('express').Router({ mergeParams: true });
const { v4: uuid } = require('uuid');
const pool = require('../db/pool');
...
router.get('/', optionalAuth, async (req, res, next) => {
  try {
    ...
    const conditions = [`p.posted_at < $1`];
    const params = [before];
    let pIdx = 2;
    ...
    if (myId) {
      ...
      const visibleUserIds = [myId, ...friendRows.map(r => r.friend_id)];
      conditions.push(`(p.user_id = ANY($${pIdx}) OR p.visibility = 'public')`);
...
router.get('/by-user/:userId', optionalAuth, async (req, res, next) => {
  try {
    const myId = req.user?.id;
    const targetId = req.params.userId;
    const limit = Math.min(parseInt(req.query.limit) || 50, 100);
    ...
    const friend = myId && (targetId === myId || await areFriends(myId, targetId));
    const visCondition = friend ? `p.visibility IN ('friends','public')` : `p.visibility = 'public'`;
...
router.get('/:id', optionalAuth, async (req, res, next) => {
  try {
    ...
    const canView = post.visibility === 'public'
      || (myId && (post.user_id === myId || await areFriends(myId, post.user_id)));
